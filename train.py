...
def gae_for(args):
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        x = torch.index_select(inx, 0, indx)
        y = torch.index_select(inx, 0, indy)
        zx = model(x)
        zy = model(y)
        ### z = model(inx)
        ### batch_pred = model.dcs(z[indx], z[indy]) # by this coding way, it takes much more training time. However, pubemed's result is worse.
